...
export default function EditHomestay({ route }) {
  ...
  const [rooms, setRooms] = useState(itemData.noOfRooms + "");
  const [price, setPrice] = useState(itemData.price + "");
  const [paymentMethod, setPaymentMethod] = useState(
    itemData.payment_method + ""
  );
  const [paymentTerm, setPaymentTerm] = useState(itemData.name);
  const [userId, setUserId] = useState(itemData.user_id);
  const [city, setCity] = useState(itemData.city_id);
  const [allCities, setAllCities] = useState();
  const [entrance, setEntrance] = useState(
    itemData.entrance == 1 ? true : false
  );
  const [graden, setGarden] = useState(itemData.graden == 1 ? true : false);
  const [furnished, setFurnished] = useState(
    itemData.furnished == 1 ? true : false
  );
  const [bathrm, setBathrm] = useState(itemData.bathrm == 1 ? true : false);
  const [parking, setParking] = useState(itemData.parking == 1 ? true : false);
  const [ac, setAC] = useState(itemData.ac == 1 ? true : false);
  const [hotWater, setHotWater] = useState(
    itemData.hotWater == 1 ? true : false
  );
  const [electricity, setElectricity] = useState(
    itemData.electricity == 1 ? true : false
  );
  const [cableTv, setCableTv] = useState(itemData.cableTv == 1 ? true : false);
  const [fibre, setFibre] = useState(itemData.fibre == 1 ? true : false);
  const [valid, setValid] = useState(itemData.valid == 1 ? true : false);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (val == true) {
        const response = await axios.post(`${BASE_URL}/editHomestay`, {
          name: name,
      ...
      <View style={styles.homeHeader}>
        ...
        <View style={styles.headerWrapper}>
          <View style={styles.container}>
            <Text style={styles.title}>Edit Homestay</Text>
          </View>
        </View>
      </View>
